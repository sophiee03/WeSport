...
// Non so se questo vada qua o nell'utenteregistrato
  router.get("/annuncio/:idCapogruppo", isAuthenticated, async (req, res) => {
    try {
      const idAnnuncio = req.params.idAnnuncio;
      const annuncio = await annuncio.findById(idCapogruppo).populate('iscritti');
      if (!annuncio) {
        return res.status(404).json({ success: false, message: "Non ci sono annunci creati da te" });
      }
      res.json(annuncio);
    } catch (error) {
